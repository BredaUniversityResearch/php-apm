...
typedef struct apm_driver {
	...
	int (* rshutdown)(void);
	zend_bool (* is_enabled)(void);
	zend_bool (* want_event)(int, int, char * );
	zend_bool (* want_stats)(void);
	int (* error_reporting)(void);
...
void apm_driver_##name##_process_stats(void); \
int apm_driver_##name##_minit(int); \
int apm_driver_##name##_rinit(void); \
int apm_driver_##name##_mshutdown(SHUTDOWN_FUNC_ARGS); \
int apm_driver_##name##_rshutdown(void); \
PHP_INI_MH(OnUpdateAPM##name##ErrorReporting) \
{ \
    APM_GLOBAL(name, error_reporting) = (apm_error_reporting_new_value : APM_E_##name); \
    return SUCCESS; \
} \
zend_bool apm_driver_##name##_is_enabled(void) \
{ \
    return APM_GLOBAL(name, enabled); \
} \
int apm_driver_##name##_error_reporting(void) \
{ \
...
zend_bool apm_driver_##name##_want_stats(void) \
{ \
    return \
        APM_GLOBAL(name, enabled) \
        && ( \
...
apm_driver_entry * apm_driver_##name##_create(void) \
{ \
